      ...
      {!paid && (
        <>
          <div className="space-y-4 mb-6">
            {items.map((item) => (
              <div key={item.beat.id} className="flex justify-between items-center border border-stone-800 rounded-xl p-4 bg-stone-900/40 gap-4">
                <div className="min-w-0">
                  <h3 className="font-bold text-orange-500 truncate" title={item.beat.title}>{item.beat.title}</h3>
                  <p className="text-sm text-stone-500">{item.license.toUpperCase()} Lease</p>
                </div>
                <div className="flex items-center gap-4 shrink-0">
                  <span className="font-bold text-orange-100 tabular-nums">KSh {item.price}</span>
                  <button onClick={() => handleRemove(item.beat.id, item.beat.title)} className="text-red-400 text-sm hover:underline focus-visible:ring-2 focus-visible:ring-red-500 rounded outline-none touch-manipulation">
                    Remove
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-green-950/30 border border-green-900/30 text-green-200 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="font-medium">M-Pesa prompt sent!</p>
                </div>
